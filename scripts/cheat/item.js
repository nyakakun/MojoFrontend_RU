...
function genItem() {
    ...
    <h2>Добавление предмета</h2>
            <label for="method">Метод выдачи:</label>
            <select id="method">
                <option value="give"/> В инвентарь </option>
                <option value="drop"/> На землю </option>
            </select>
            <label for="item-search">Поиск предмета:</label>
                <div style="display: flex; flex-direction: column;">
                    <div style="display: flex; align-items: center; overflow: hidden;">
                        <input id="item-search" style="flex: 4" type="text" placeholder="Поиск по имени" />
                        <button id="clear" style="margin-left: 0.25em; transition: all ease-in-out 0.5s; flex: 0; opacity: 0;">Очистить</button>
                    </div>
                    <div id="name-list" style="overflow-y: auto; overflow-x: hidden; max-height: 10em;height: 100%; transition: all ease-in-out 0.5s;">
                    </div>
                </div>
            <label for="amount">Количество:</label><input type="number" id="amount" name="amount" value=100 />
            <input type="hidden" id="item-id" />
            <button disabled id="execute">Добавить</button>
    </div>`;
    updateItemList();
